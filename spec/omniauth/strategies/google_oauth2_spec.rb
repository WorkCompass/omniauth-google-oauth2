...
describe OmniAuth::Strategies::GoogleOauth2 do
  ...
  describe '#client_options' do
    ...
    it 'has correct base_api_url' do
      expect(subject.client.options[:base_api_url]).to eq('https://www.googleapis.com')
    end

    it 'has correct base_auth_domain' do
      expect(subject.client.options[:base_auth_domain]).to eq('accounts.google.com')
    end
    ...
    describe 'overrides' do
      context 'as strings' do
        ...
        it 'should allow overriding the base_api_url' do
          @options = { client_options: { 'base_api_url' => 'https://example.com' } }
          expect(subject.client.options[:base_api_url]).to eq('https://example.com')
        end

        it 'should allow overriding the base_auth_domain' do
          @options = { client_options: { 'base_auth_domain' => 'accounts.example.com' } }
          expect(subject.client.options[:base_auth_domain]).to eq('accounts.example.com')
        end
      end
      ...
      context 'as symbols' do
        ...
        it 'should allow overriding the base_auth_domain' do
          @options = { client_options: { base_auth_domain: 'https://example.com' } }
          expect(subject.client.options[:base_auth_domain]).to eq('https://example.com')
        end
      end
    end
  end
